...
export function OnboardingWizard() {
  ...
  async function runAdapterEnvironmentTest(
    ...
    try {
      ...
      setAdapterEnvError(
        err instanceof Error ? err.message : "Adapter 环境测试失败"
      );
      return null;
    } finally {
      setAdapterEnvLoading(false);
  ...
  async function handleStep2Next() {
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (!selectedModelId) {
          setError(
            "OpenCode 需要显式指定 provider/model 格式的模型。"
          );
          return;
        }
        ...
        if (!discoveredModels.some((entry) => entry.id === selectedModelId)) {
          setError(
            discoveredModels.length === 0
              ? "未发现 OpenCode 模型。请先运行 `opencode models` 并完成 provider 登录。"
              : `当前配置的 OpenCode 模型不可用：${selectedModelId}`
          );
          return;
        }
      }
  ...
  async function handleUnsetAnthropicApiKey() {
    ...
    try {
      ...
      if (result?.status === "fail") {
        setError(
          "已在 adapter config 中清空 ANTHROPIC_API_KEY 并重试，但环境测试仍然失败。"
        );
      }
    } catch (err) {
      setError(
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">创建第一个 Agent</h3>
                      <p className="text-xs text-muted-foreground">
                        选择这个 Agent 的运行方式。
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        {[
                          ...
                          },
                          {
                            ...
                            icon: Bot,
                            desc: "通过 gateway protocol 调用 OpenClaw",
                            comingSoon: true,
                            disabledLabel: "请在应用内配置 OpenClaw"
                          }
                        ].map((opt) => (
                          <button
                            key={opt.value}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            运行一次实时探测，请 adapter CLI 返回 hello。
                          </p>
                        </div>
                        <Button
                          size="sm"
                      ...
                      {shouldSuggestUnsetAnthropicApiKey && (
                        <div className="rounded-md border border-amber-300/60 bg-amber-50/40 px-2.5 py-2 space-y-2">
                          <p className="text-[11px] text-amber-900/90 leading-relaxed">
                            ...
                            已设置时测试失败。你可以在这个 CEO adapter config 中清空它并重新探测。
                          </p>
                          <Button
                            size="sm"
                            variant="outline"
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">给它一个起始任务</h3>
                      <p className="text-xs text-muted-foreground">
                        给 Agent 一个小任务开始工作，例如修复 bug、做研究或写脚本。
                      </p>
                    </div>
                  </div>
                  <div>
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        一切都已准备好。现在启动会创建起始任务、唤醒 Agent，并打开对应 Issue。
                      </p>
                    </div>
                  </div>
