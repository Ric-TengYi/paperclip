...
function SkillList({ values }: { values: unknown }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">技能 Skills</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      ...
      <PayloadField label="头衔" value={payload.title} />
    ...
    <div className="mt-3 space-y-1.5 text-sm">
      ...
      {(budgetAmount !== null || observedAmount !== null) ? (
        <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          上限 {budgetAmount !== null ? formatCents(budgetAmount) : "—"} · 已观察到 {observedAmount !== null ? formatCents(observedAmount) : "—"}
        </div>
      ) : null}
      {!!payload.guidance && (
