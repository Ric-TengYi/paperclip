...
const TimelineList = memo(function TimelineList({
  ...
  if (timeline.length === 0) {
    return <p className="text-sm text-muted-foreground">还没有评论或运行记录。</p>;
  }
          ...
          return (
            <div key={`run:${run.runId}`} className="border border-border bg-accent/20 p-3 overflow-hidden min-w-0 rounded-sm">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">运行 Run</span>
                <Link
...
export function CommentThread({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold">评论与运行记录 ({timeline.length})</h3>
