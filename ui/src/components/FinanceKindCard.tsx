  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        ...
        <CardDescription>按事件类型分组展示账户级费用。</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">该时间范围内还没有财务事件。</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}
