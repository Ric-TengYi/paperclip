...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "生命周期预算" : "按月 UTC 预算";
}
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `已达到上限的 ${summary.utilizationPercent}%` : "未配置上限"}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          在 {summary.warnPercent}% 时触发软告警{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} 暂停` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>剩余</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "不限"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "该项目的执行已暂停，直到提高预算或解除该事件。"
          : "此范围内的 heartbeat 已暂停，直到提高预算或解除该事件。"}
      </div>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "已暂停" : summary.status === "warning" ? "告警" : summary.status === "hard_stop" ? "硬停止" : "健康"}
          </div>
        </div>
