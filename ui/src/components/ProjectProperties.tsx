      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                Repo 用于标识代码来源。本地目录是 agents 默认写入代码的位置。
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">仓库 Repo</div>
              {codebase.repoUrl ? (
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">本地目录</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">由 Paperclip 管理的目录。</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? "更改本地目录" : "设置本地目录"}
                  </Button>
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                该项目还存在额外的历史 workspace 记录。Paperclip 当前使用主 workspace 作为代码库视图。
              </div>
            )}
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">保存 workspace 失败。</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">删除 workspace 失败。</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">更新 workspace 失败。</p>
          )}
        </div>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    项目级默认设置，用于控制独立的 issue checkout 与执行工作区行为。
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>启用独立的 issue checkout</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      允许 issues 在项目主 checkout 与独立执行工作区之间进行选择。
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <button
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>新建 issues 默认使用独立 checkout</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          若关闭，新建 issues 会默认停留在项目主 checkout，除非有人手动启用。
                        </div>
                      </div>
                      <button
                        data-slot="toggle"
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          主机托管实现：<span className="text-foreground">Git worktree</span>
                        </div>
                        <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          Provision 会在派生出的 worktree 内、Agent 执行前运行。Teardown 会保存在这里，供后续清理流程使用。
                        </p>
                      </div>
                    ) : null}
                  </div>
