...
export function NewAgent() {
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (!selectedModel) {
        setFormError("OpenCode 需要显式指定 provider/model 格式的模型。");
        return;
      }
      ...
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError("OpenCode 模型仍在加载中，请稍后再试。");
        return;
      }
      ...
      <div className="border border-border">
        ...
        <div className="px-4 pb-2">
          <input
            ...
            placeholder="职位，例如 VP of Engineering"
            value={title}
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">公司技能 / Company Skills</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                从公司技能库中选择可选技能。内置的 Paperclip runtime skills 会自动附加。
              </p>
            </div>
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">这会成为你的 CEO Agent</p>
          )}
          {formError && (
