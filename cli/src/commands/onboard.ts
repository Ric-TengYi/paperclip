...
function quickstartDefaultsFromEnv(): {
  ...
  if (deploymentMode === "local_trusted" && process.env.PAPERCLIP_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      ...
      reason: "已忽略，因为 local_trusted 模式会强制使用 private exposure",
    });
  }
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (opts.yes) {
    p.log.message(pc.dim("已启用 `--yes`：使用 Quickstart 默认值。"));
  } else {
      ...
      options: [
        {
          ...
          label: "Quickstart",
          hint: "推荐：使用本地默认配置并立即可运行",
        },
        {
          value: "advanced" as const,
          label: "高级设置",
          hint: "自定义数据库、server、storage 等配置",
        },
      ],
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("当前无法连接数据库，你稍后可以用 `paperclipai doctor` 修复"));
      }
    }
      ...
      try {
        if (llm.provider === "claude") {
          ...
          if (res.ok || res.status === 400) {
            ...
            s.stop(pc.yellow("API key 看起来无效，稍后可以再更新"));
          } else {
            s.stop(pc.yellow("暂时无法校验 API key，继续执行"));
          }
        } else {
        ...
        s.stop(pc.yellow("暂时无法访问 API，继续执行"));
      }
    }
    ...
    p.log.message(
      pc.dim(
        `使用默认值：provider=${secrets.provider}, strictMode=${secrets.strictMode}, keyFile=${secrets.localEncrypted.keyFilePath}`,
      ),
    );
  } else {
    p.log.step(pc.bold("Quickstart"));
    p.log.message(pc.dim("正在使用 Quickstart 默认配置。"));
    if (usedEnvKeys.length > 0) {
      p.log.message(pc.dim(`已启用环境感知默认值（检测到 ${usedEnvKeys.length} 个 env var）。`));
    } else {
      p.log.message(
        pc.dim("未检测到环境覆盖项：将使用 embedded 数据库、文件存储和本地加密 secrets。"),
      );
    }
  ...
  if (jwtSecret.created) {
    ...
    p.log.info(`正在使用环境变量中的 ${pc.cyan("PAPERCLIP_AGENT_JWT_SECRET")}`);
  } else {
    p.log.info(`正在使用 ${pc.dim(envFilePath)} 中已有的 ${pc.cyan("PAPERCLIP_AGENT_JWT_SECRET")}`);
  }
  ...
  p.note(
    [
      ...
      `服务：${server.deploymentMode}/${server.exposure} @ ${server.host}:${server.port}`,
      `允许的 hosts：${server.allowedHostnames.length > 0 ? server.allowedHostnames.join(", ") : "（仅 loopback）"}`,
      `Auth URL 模式：${auth.baseUrlMode}${auth.publicBaseUrl ? ` (${auth.publicBaseUrl})` : ""}`,
      `存储：${storage.provider}`,
      `Secrets：${secrets.provider}（strict mode ${secrets.strictMode ? "on" : "off"}）`,
      "Agent 认证：PAPERCLIP_AGENT_JWT_SECRET 已配置",
    ].join("\n"),
    "配置已保存",
  );
  ...
  p.note(
    [
      ...
      `诊断当前安装：${pc.cyan("paperclipai doctor")}`,
    ].join("\n"),
    "下一步命令",
  );

  if (canCreateBootstrapInviteImmediately({ database, server })) {
    p.log.step("正在生成 bootstrap CEO 邀请");
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        "Bootstrap CEO 邀请会在服务启动后创建。",
        `下一步：${pc.cyan("paperclipai run")}`,
        `然后执行：${pc.cyan("paperclipai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
