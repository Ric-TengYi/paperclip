...
export async function promptDatabase(current?: DatabaseConfig): Promise<DatabaseConfig> {
  ...
  const mode = await p.select({
    message: "数据库模式",
    options: [
      { value: "embedded-postgres" as const, label: "Embedded PostgreSQL（本地托管）", hint: "推荐" },
      { value: "postgres" as const, label: "PostgreSQL（外部服务）" },
    ],
  ...
  if (mode === "postgres") {
    const value = await p.text({
      ...
      validate: (val) => {
        if (!val) return "PostgreSQL 模式必须填写连接串";
    ...
    const dataDir = await p.text({
      message: "Embedded PostgreSQL 数据目录",
    ...
    const portValue = await p.text({
      message: "Embedded PostgreSQL 端口",
  ...
  const backupDirInput = await p.text({
    ...
    validate: (val) => (!val || val.trim().length === 0 ? "备份目录不能为空" : undefined),
  });
