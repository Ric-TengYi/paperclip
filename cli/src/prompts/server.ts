    ...
    options: [
      {
        value: "local_trusted",
        label: "Local trusted",
        hint: "最省事的本地模式（无需登录，仅限 localhost）",
      },
      {
        value: "authenticated",
        label: "Authenticated",
    ...
    const allowedHostnamesInput = await p.text({
      ...
      validate: (val) => {
        try {
          ...
          return err instanceof Error ? err.message : "hostname 列表无效";
        }
      },
    });
  ...
  if (deploymentMode === "authenticated" && exposure === "public") {
    const urlInput = await p.text({
      ...
      validate: (val) => {
        ...
        if (!candidate) return "公网部署必须填写公开 base URL";
        try {
